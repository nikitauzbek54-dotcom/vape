...
async def cmd_start(message: Message, db):
    text = (
        f"👋 Добро пожаловать в <b>CLOUD SHOP</b>!\n\n"
...
async def cart_qty(call: CallbackQuery, db):
    parts = call.data.replace("qty_", "").split("_")
    cart_id = int(parts[0])
    delta = int(parts[1])
    await db.update_cart_qty(cart_id, delta)
    items = await db.get_cart(call.from_user.id)
    total = sum(i["price"] * i["quantity"] for i in items)
...
async def start_checkout(call: CallbackQuery, db, state: FSMContext):
    ...
    try:
        await call.message.edit_text("🚚 <b>Как получить заказ?</b>", reply_markup=delivery_menu())
    except Exception:
        await call.message.answer("🚚 <b>Как получить заказ?</b>", reply_markup=delivery_menu())
    await call.answer()
...
async def choose_courier(call: CallbackQuery, state: FSMContext):
    ...
    try:
        await call.message.edit_text("📍 <b>Выбери станцию метро:</b>", reply_markup=metro_menu())
    except Exception:
        await call.message.answer("📍 <b>Выбери станцию метро:</b>", reply_markup=metro_menu())
    await call.answer()
...
async def got_comment(message: Message, state: FSMContext, db):
    ...
    lines.append(f"🚚 {'Курьер' if data['delivery'] == 'courier' else 'Самовывоз'}")
    if data.get("metro"):
...
async def about(call: CallbackQuery, db):
    about_text = await db.get_setting("about_text", "CLOUD SHOP — вейп-шоп в Новосибирске.")
